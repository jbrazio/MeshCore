...
/// This 2D array has the characters for the sets USX_ALPHA, USX_SYM and USX_NUM. Where a character cannot fit
/// into a uint8_t, 0 is used and handled in code.
...
/// Vertical Codes and Set number for frequent sequences in sets USX_SYM and USX_NUM. First 3 bits indicate
/// set (USX_SYM/USX_NUM) and rest are vcode positions
uint8_t usx_freq_codes[] = { (1 << 5) + 25, (1 << 5) + 26, (1 << 5) + 27,
                             (2 << 5) + 23, (2 << 5) + 24, (2 << 5) + 25 };
  ...
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 28; j++) {
      ...
      if (c > 32) {
        ...
        if (c >= 'a' && c <= 'z') usx_code_94[c - USX_OFFSET_94 - ('a' - 'A')] = (i << 5) + j;
      }
    }
  }
  is_inited = 1;
...
int append_code(char *out, int olen, int ol, uint8_t code, uint8_t *state, const uint8_t usx_hcodes[],
  ...
  if (!usx_hcode_lens[hcode] && hcode != USX_ALPHA) return ol;
  switch (hcode) {
  case USX_ALPHA:
    if (*state != USX_ALPHA) {
  ...
  case USX_NUM:
    if (*state != USX_NUM) {
      ...
      if (usx_sets[hcode][vcode] >= '0' && usx_sets[hcode][vcode] <= '9') *state = USX_NUM;
    }
  }
...
int matchOccurance(const char *in, int len, int l, char *out, int olen, int *ol, uint8_t *state,
...
/// This is a crude implementation that is not optimized.  Assuming only short strings \n
/// are encoded, this is not much of an issue.
int matchLine(const char *in, int len, int l, char *out, int olen, int *ol, struct us_lnk_lst *prev_lines,
              uint8_t *state, const uint8_t usx_hcodes[], const uint8_t usx_hcode_lens[]) {
  int last_ol = *ol;
  int last_len = 0;
  int last_dist = 0;
  int last_ctx = 0;
  ...
  do {
    ...
    for (; j < limit; j++) {
      for (i = l, k = j; k < line_len && i < len; k++, i++) {
        if (prev_lines->data[k] != in[i]) break;
      }
...
int append_nibble_escape(char *out, int olen, int ol, uint8_t state, const uint8_t usx_hcodes[],
                         const uint8_t usx_hcode_lens[]) {
  SAFE_APPEND_BITS(ol = append_switch_code(out, olen, ol, state));
  SAFE_APPEND_BITS(ol = append_bits(out, olen, ol, usx_hcodes[USX_NUM], usx_hcode_lens[USX_NUM]));
...
/// Appends the terminator code depending on the state, preset and whether full terminator needs to be encoded
/// to out or not \n
int append_final_bits(char *const out, const int olen, int ol, const uint8_t state,
                      const uint8_t is_all_upper, const uint8_t usx_hcodes[],
                      const uint8_t usx_hcode_lens[]) {
...
/// Macro used in the main compress function so that if the output len exceeds given maximum length (olen) it
/// can exit
...
int unishox2_compress_lines(const char *in, int len, UNISHOX_API_OUT_AND_LEN(char *out, int olen),
                            const uint8_t usx_hcodes[], const uint8_t usx_hcode_lens[],
  ...
  is_all_upper = 0;
  SAFE_APPEND_BITS2(
      rawolen, ol = append_bits(out, olen, ol, UNISHOX_MAGIC_BITS, UNISHOX_MAGIC_BIT_LEN)); // magic bit(s)
  for (l = 0; l < len; l++) {
    ...
    if (l && len > 4 && l < (len - 4) && usx_hcode_lens[USX_NUM]) {
      if (c_in == in[l - 1] && c_in == in[l + 1] && c_in == in[l + 2] && c_in == in[l + 3]) {
        ...
        SAFE_APPEND_BITS2(rawolen,
                          ol = append_code(out, olen, ol, RPT_CODE, &state, usx_hcodes, usx_hcode_lens));
    ...
    if (l <= (len - 36) && usx_hcode_lens[USX_NUM]) {
      if (in[l + 8] == '-' && in[l + 13] == '-' && in[l + 18] == '-' && in[l + 23] == '-') {
        ...
        for (; uid_pos < l + 36; uid_pos++) {
          ...
          if (c_uid == '-' && (uid_pos == 8 || uid_pos == 13 || uid_pos == 18 || uid_pos == 23)) continue;
    ...
    if (l < (len - 5) && usx_hcode_lens[USX_NUM]) {
      ...
      if ((hex_type == USX_NIB_HEX_LOWER || hex_type == USX_NIB_HEX_UPPER) && hex_len > 3) {
        SAFE_APPEND_BITS2(rawolen,
                          ol = append_nibble_escape(out, olen, ol, state, usx_hcodes, usx_hcode_lens));
        SAFE_APPEND_BITS2(rawolen,
                          ol = append_bits(out, olen, ol, (hex_type == USX_NIB_HEX_LOWER ? 0x80 : 0xE0),
        ...
        if (usx_templates[i]) {
          ...
          for (; j < rem && l + j < len; j++) {
            ...
            if (c_t == 'f' || c_t == 'F') {
              ...
              if (c_in < '0' || c_in > (c_t == 'r' ? '7' : (c_t == 't' ? '3' : '1'))) break;
            } else if (c_t != c_in)
              break;
          }
          if (((float)j / rem) > 0.66) {
            ...
            SAFE_APPEND_BITS2(
                rawolen, ol = append_bits(out, olen, ol, (count_codes[i] & 0xF8), count_codes[i] & 0x07));
            ...
            for (int k = 0; k < j; k++) {
              ...
              if (c_t == 'f' || c_t == 'F')
                ...
                SAFE_APPEND_BITS2(rawolen,
                                  ol = append_bits(out, olen, ol, (in[l + k] - '0') << (8 - c_t), c_t));
              }
            }
            l += j;
            l--;
      ...
      for (i = 0; i < 6; i++) {
        ...
        if (len - seq_len >= 0 && l <= len - seq_len) {
          if (memcmp(usx_freq_seq[i], in + l, seq_len) == 0 && usx_hcode_lens[usx_freq_codes[i] >> 5]) {
            SAFE_APPEND_BITS2(rawolen, ol = append_code(out, olen, ol, usx_freq_codes[i], &state, usx_hcodes,
                                                        usx_hcode_lens));
            l += seq_len;
            l--;
            break;
    ...
    c_next = 0;
    if (l + 1 < len) c_next = in[l + 1];

    if (c_in >= 32 && c_in <= 126) {
      if (is_upper && !is_all_upper) {
        for (ll = l + 4; ll >= l && ll < len; ll--) {
          if (in[ll] < 'A' || in[ll] > 'Z') break;
        }
        if (ll == l - 1) {
          SAFE_APPEND_BITS2(rawolen, ol = append_switch_code(out, olen, ol, state));
          SAFE_APPEND_BITS2(
              rawolen, ol = append_bits(out, olen, ol, usx_hcodes[USX_ALPHA], usx_hcode_lens[USX_ALPHA]));
          state = USX_ALPHA;
          is_all_upper = 1;
        }
      }
      ...
      if (c_in == 0) {
        ...
        c_in--;
        SAFE_APPEND_BITS2(rawolen, ol = append_code(out, olen, ol, usx_code_94[(int)c_in], &state, usx_hcodes,
                                                    usx_hcode_lens));
      }
    } else if (c_in == 13 && c_next == 10) {
      SAFE_APPEND_BITS2(rawolen,
                        ol = append_code(out, olen, ol, CRLF_CODE, &state, usx_hcodes, usx_hcode_lens));
      l++;
    } else if (c_in == 10) {
      if (state == USX_DELTA) {
        SAFE_APPEND_BITS2(rawolen,
        ...
        SAFE_APPEND_BITS2(rawolen,
                          ol = append_code(out, olen, ol, LF_CODE, &state, usx_hcodes, usx_hcode_lens));
    } else if (c_in == 13) {
      SAFE_APPEND_BITS2(rawolen,
                        ol = append_code(out, olen, ol, CR_CODE, &state, usx_hcodes, usx_hcode_lens));
    } else if (c_in == '\t') {
      SAFE_APPEND_BITS2(rawolen,
                        ol = append_code(out, olen, ol, TAB_CODE, &state, usx_hcodes, usx_hcode_lens));
    } else {
      int utf8len;
      ...
      if (uni) {
        ...
        if (state != USX_DELTA) {
          ...
          if (uni2) {
            ...
            SAFE_APPEND_BITS2(
                rawolen, ol = append_bits(out, olen, ol, usx_hcodes[USX_ALPHA], usx_hcode_lens[USX_ALPHA]));
            SAFE_APPEND_BITS2(rawolen, ol = append_bits(out, olen, ol, usx_vcodes[1],
                                                        usx_vcode_lens[1])); // code for space (' ')
            state = USX_DELTA;
          } else {
            SAFE_APPEND_BITS2(rawolen, ol = append_switch_code(out, olen, ol, state));
            SAFE_APPEND_BITS2(
                rawolen, ol = append_bits(out, olen, ol, usx_hcodes[USX_DELTA], usx_hcode_lens[USX_DELTA]));
          }
        }
        ...
        for (int bi = l + 1; bi < len; bi++) {
          ...
          //   break;
          if (readUTF8(in, len, bi, &utf8len)) break;
...
// Main API function. See unishox2.h for documentation
int unishox2_compress(const char *in, int len, UNISHOX_API_OUT_AND_LEN(char *out, int olen),
                      const uint8_t usx_hcodes[], const uint8_t usx_hcode_lens[], const char *usx_freq_seq[],
                      const char *usx_templates[]) {
  return unishox2_compress_lines(in, len, UNISHOX_API_OUT_AND_LEN(out, olen), usx_hcodes, usx_hcode_lens,
                                 usx_freq_seq, usx_templates, NULL);
}

// Main API function. See unishox2.h for documentation
int unishox2_compress_simple(const char *in, int len, char *out) {
  return unishox2_compress_lines(in, len, UNISHOX_API_OUT_AND_LEN(out, INT_MAX - 1), USX_HCODES_DFLT,
                                 USX_HCODE_LENS_DFLT, USX_FREQ_SEQ_DFLT, USX_TEMPLATES, NULL);
}
  ...
  if (*bit_no_p < len) {
    ...
    do {
      if (code <= usx_vsections[i]) {
        uint8_t vcode =
            usx_vcode_lookup[usx_vsection_pos[i] + ((code & usx_vsection_mask[i]) >> usx_vsection_shift[i])];
        (*bit_no_p) += ((vcode >> 5) + 1);
        if (*bit_no_p > len) return 99;
        return vcode & 0x1F;
      }
...
int readHCodeIdx(const char *in, int len, int *bit_no_p, const uint8_t usx_hcodes[],
                 const uint8_t usx_hcode_lens[]) {
  if (!usx_hcode_lens[USX_ALPHA]) return USX_ALPHA;
  if (*bit_no_p < len) {
    uint8_t code = read8bitCode(in, len, *bit_no_p);
...
int32_t readCount(const char *in, int *bit_no_p, int len) {
  ...
  if (idx == 99) return -1;
  if (*bit_no_p + count_bit_lens[idx] - 1 >= len) return -1;
...
int32_t readUnicode(const char *in, int *bit_no_p, int len) {
  ...
  if (idx >= 0) {
    ...
    (*bit_no_p)++;
    if (*bit_no_p + uni_bit_len[idx] - 1 >= len) return 0x7FFFFF00 + 99;
...
int decodeRepeat(const char *in, int len, char *out, int olen, int ol, int *bit_no,
  ...
  if (prev_lines) {
    ...
    if (cur_line == NULL) return -1;
    if (left <= 0) return olen + 1;
    if (dist >= strlen(cur_line->data)) return -1;
...
// Main API function. See unishox2.h for documentation
int unishox2_decompress_lines(const char *in, int len, UNISHOX_API_OUT_AND_LEN(char *out, int olen),
                              const uint8_t usx_hcodes[], const uint8_t usx_hcode_lens[],
                              const char *usx_freq_seq[], const char *usx_templates[],
                              struct us_lnk_lst *prev_lines) {
  ...
  while (bit_no < len) {
      ...
      if ((delta >> 8) == 0x7FFFFF) {
        ...
        case 1:
          ...
          if (h == USX_DICT) {
            ...
            if (rpt_ret < 0) return ol; // if we break here it will only break out of switch
    ...
    if (v == 0 && h != USX_SYM) {
      ...
      if (h == USX_ALPHA) {
        ...
        if (h != USX_NUM || dstate != USX_DELTA) v = readVCodeIdx(in, len, &bit_no);
        if (v == 99) {
          bit_no = orig_bit_no;
          break;
        }
    ...
    if (c >= 'a' && c <= 'z') {
      ...
      if (c >= '0' && c <= '9') {
        ...
        if (v == 8) {
          ...
          if (count < 0) break;
          count += 4;
          if (ol <= 0) return 0; // invalid encoding
          char rpt_c = out[ol - 1];
          while (count--)
...
// Main API function. See unishox2.h for documentation
int unishox2_decompress(const char *in, int len, UNISHOX_API_OUT_AND_LEN(char *out, int olen),
                        const uint8_t usx_hcodes[], const uint8_t usx_hcode_lens[],
                        const char *usx_freq_seq[], const char *usx_templates[]) {
  return unishox2_decompress_lines(in, len, UNISHOX_API_OUT_AND_LEN(out, olen), usx_hcodes, usx_hcode_lens,
                                   usx_freq_seq, usx_templates, NULL);
}
